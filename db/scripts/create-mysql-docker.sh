...
if [ -x "$(command -v docker)" ]; then
    if [[ "$(command docker -v 2> /dev/null)" == *"Docker version"* ]]; then
        ...
        if [[ "$(docker images -q mysql/mysql-server 2> /dev/null)" == "" ]]; then
        ...
        if [[ "$(docker inspect -f '{{.State.Running}}' coffee-devdb 2> /dev/null)" == "true" ]]; then
            ...
            docker run --name=coffee-devdb -p 3306:3306 -e MYSQL_ROOT_HOST=% -e MYSQL_ROOT_PASSWORD=root -d mysql/mysql-server:latest
        fi
    else
